...
# v4.11: de services die deze integratie registreert, zodat ze met de
# LAATSTE config-entry weer verdwijnen. Ze werden bij het opzetten
# geregistreerd en nooit verwijderd; na het opruimen wezen ze naar een
# coordinator die er niet meer was.
EIGEN_SERVICES = (
...
async def async_unload_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    if unload_ok:
        ...
        # v4.11: en de services, maar alleen met de LAATSTE entry. Ze
        # werden bij het opzetten geregistreerd en nooit verwijderd; na
        # het opruimen wezen ze naar een coordinator die er niet meer
        # was. Met twee entries mag het opruimen van de ene de services
        # van de andere niet weghalen - vandaar de telling.
        resterend = [
            sleutel
            for sleutel in hass.data.get(DOMAIN, {})
            if isinstance(sleutel, str) and not sleutel.endswith("_solar_tracker")
        ]
        if not resterend:
            for naam in EIGEN_SERVICES:
                if hass.services.has_service(DOMAIN, naam):
                    hass.services.async_remove(DOMAIN, naam)
