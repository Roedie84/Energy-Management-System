...
# --- v4.12: de reserve is een verkooppoort, geen bodem -----------------
#
# Uit doorloop 5 van de audit. `gemist_door_reserve_eur` rekende de
# beste planning nog een keer MET de reserve als harde bodem, en nam het
# verschil. Dat bedrag kwam op 146 euro per jaar uit, en dat is fout om
# twee redenen.
#
# Ten eerste is de reserve in het EMS geen bodem maar een POORT: het
# huis mag altijd uit de accu worden gedekt, ook onder de reserve;
# alleen leveren aan het NET is verboden zolang de voorraad eronder zit
# (`may_sell_now`). Een bodem verbiedt ook het dekken van het huis, en
# dat is een veel zwaardere beperking.
#
# Ten tweede is de reserve op deze dagen fysiek niet te halen zonder uit
# het net te laden. Als bodem maakte hij het probleem daardoor soms
# onoplosbaar, en dan viel het hele bedrag op de reserve.
#
# Met de poort als model is het verschil ongeveer 2 cent per dag in
# plaats van 40. De energie die de reserve vasthoudt wordt namelijk niet
# verspild - die gaat 's nachts naar het huis tegen 30 tot 37 ct.


def _avonddag():
    """Zestien kwartieren: goedkoop, dan een dure avond. Geen zon, dus
    de accu kan alleen leveren wat er in zit."""
    prijzen = [0.20] * 8 + [0.60] * 4 + [0.25] * 4
    return [
        Kwartier(huis_kwh=0.1, pv_kwh=0.0, prijs_eur=p, accu_kwh=0.0)
        for p in prijzen
    ]
...
def test_de_poort_verbiedt_leveren_maar_niet_het_huis():
    from custom_components.energy_management_system.nabeschouwing import (
        beste_planning,
    )

    kw = _avonddag()
    # reserve boven de beginvoorraad: verkopen mag niet, het huis dekken wel
    met_poort = beste_planning(
        kw, verkoop_onder_reserve_verboden=True, reserve_kwh=1.5, **ACCU
    )

    assert met_poort["te_becijferen"] is True
    # het huis is gedekt: er is ontladen
    assert sum(a["ontladen_kwh"] for a in met_poort["acties"]) > 0


def test_een_bodem_is_veel_strenger_dan_een_poort():
    """Waarom het oude model te hoge bedragen gaf: een bodem verbiedt
    ook het DEKKEN VAN HET HUIS onder de reserve, een poort alleen het
    leveren aan het net. De bodem komt daardoor veel duurder uit, en dat
    hele verschil werd aan de reserve toegeschreven.
    """
    from custom_components.energy_management_system.nabeschouwing import (
        beste_planning,
    )
    ...
    kw = _avonddag()
    met_bodem = beste_planning(kw, **{**ACCU, "bodem_kwh": 1.5})
    met_poort = beste_planning(
        kw, verkoop_onder_reserve_verboden=True, reserve_kwh=1.5, **ACCU
    )

    assert met_poort["kosten_eur"] < met_bodem["kosten_eur"]


def test_de_reserve_kost_veel_minder_dan_als_bodem():
    kw = _avonddag()
    uit = nabeschouwing(
        kw, eind_kwh=0.5, tijdstippen=[str(i) for i in range(16)],
        slijtage_eur_per_kwh=0.0, reserve_kwh=1.5, **ACCU,
    )

    assert uit["gemist_door_reserve_eur"] <= uit["gemist_eur"]
    assert uit["reserve_als"] == "verkooppoort"


def test_de_splitsing_telt_nog_steeds_op():
    kw = _avonddag()
    uit = nabeschouwing(
        kw, eind_kwh=0.5, tijdstippen=[str(i) for i in range(16)],
        slijtage_eur_per_kwh=0.0, reserve_kwh=1.5, **ACCU,
    ...
    assert uit["gemist_door_reserve_eur"] + uit["gemist_door_voorspelling_eur"] == pytest.approx(
        max(0.0, uit["gemist_eur"]), abs=0.01
    )
